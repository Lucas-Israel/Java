...
public class AuthorController {
  ...
  public ResponseEntity<ResponseDTO<Author>> createAuthor(@RequestBody AuthorDTO authorDTO) {
    Author newAuthor = authorService.inserAuthor(authorDTO.toAuthor());
  ...
  public ResponseEntity<ResponseDTO<Author>> updateAuthor(@PathVariable Long authorId,
    ...
    if (authorOptional.isEmpty()) {
      ResponseDTO<Author> responseDTO = new ResponseDTO<>(
          String.format("Não foi possivel encontrar o autor de ID %d", authorId), null
      );
      return ResponseEntity.status(HttpStatus.NOT_FOUND).body(responseDTO);
    }

    ResponseDTO<Author> responseDTO = new ResponseDTO<>("Autor atualizado com sucesso", authorOptional.get());
  ...
  public ResponseEntity<ResponseDTO<Author>> removeAuthor(@PathVariable long authorId) {
    ...
    ResponseDTO<Author> responseDTO = new ResponseDTO<>(
        "Author removido com sucesso", null
    );
    return ResponseEntity.ok(responseDTO);
  }
