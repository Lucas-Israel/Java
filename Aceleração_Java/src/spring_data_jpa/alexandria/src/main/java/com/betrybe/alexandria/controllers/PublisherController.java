...
public class PublisherController {
  ...
  public ResponseEntity<ResponseDTO<Publisher>> removePublisher(@PathVariable Long publisherId) {
    Optional<Publisher> publisherOptional = publisherService.removePublisher(publisherId);
  ...
  public ResponseEntity<ResponseDTO<Publisher>> getPublisherById(@PathVariable Long publisherId) {
    ...
    if (optionalPublisher.isEmpty()) {
      ResponseDTO<Publisher> responseDTO = new ResponseDTO<>(
          String.format("Não foi possivel encontrar o livro de ID %d", publisherId), null
      );
      return ResponseEntity.status(HttpStatus.NOT_FOUND).body(responseDTO);
    }
